...
def socios():
    conn = mysql.connect()   #se conecta a la conexion mysql.init_app(app)
    cursor = conn.cursor()   #almacena lo que ejecutamos
    cursor.execute("SELECT * FROM `biblioteca`.`socios`")    #ejecuta la sentencia SQL sobre el id
...
def prestar(id):
    conn = mysql.connect()   #se conecta a la conexion mysql.init_app(app)
    cursor = conn.cursor()   #almacena lo que ejecutamos
    cursor.execute("SELECT * FROM `biblioteca`.`inventario` WHERE id_stock=%s", (id))     #ejecuta la sentencia SQL sobre el id
    libros=cursor.fetchall()    #traemos toda la informacion
    conn.commit()    #cerramos la conexion
...
def storage_socio():
    ...
    sql = "INSERT INTO `biblioteca`.`socios` (`id_socio`,`nombre_socio`,`correo_socio`) VALUES (null,%s,%s);"
...
def destroy_libro(id):
    ...
    cursor.execute("SELECT url_foto FROM `biblioteca`.`inventario` WHERE id_stock=%s",(id))
    fila=cursor.fetchall()
    os.remove(os.path.join(app.config['CARPETA'],fila[0][0]))

    cursor.execute("DELETE FROM `biblioteca`.`inventario` WHERE id_stock=%s", (id))   #en vez de pasarle el string la escribimos
    conn.commit()      #cerramos la conexion
...
def update():
    ...
    if _foto.filename !='':
        ...
        os.remove(os.path.join(app.config['CARPETA'],fila[0][0]))
        cursor.execute("UPDATE `biblioteca`.`inventario` SET url_foto=%s WHERE id_stock=%s",(nuevoNombreFoto,id))
        conn.commit()
